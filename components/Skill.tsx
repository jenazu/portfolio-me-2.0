...
export default function Skill({ directionLeft,skill }: Props) {
  return (
    <div className="group relative flex cursor-pointer">
      ...
      />
      <div
        className="absolute opacity-0 group-hover:opacity-80 duration-300 ease-in-out 
      group-hover:bg-white h-24 w-24 md:w-28 md:h-28 xl:h-32 xl:w-32 rounded-full z-0"
      >
        <div className="flex items-center justify-center h-full">
          <p className="text-xl max-w-sm font-bold text-black opacity-100">{skill.title}</p>
        </div>
      </div>
    </div>
  );
